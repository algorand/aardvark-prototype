...
// Less defines a total ordering on addresses, returning false
// if the subject address is less than its argument.
//
// Less compares addresses lexicographically.
func (addr Address) Less(addr0 Address) bool {
	return bytes.Compare(addr[:], addr0[:]) < 0
}
