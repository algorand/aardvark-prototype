...
type txBacklogMsg struct {
	...
	proof           transactions.TransactionProof
...
func (handler *TxHandler) backlogWorker() {
	...
	for {
		...
		case wi, ok := <-handler.postVerificationQueue:
			...
			err := handler.txPool.Remember(*verifiedTxn, wi.proof)
			if err != nil {
...
func (handler *TxHandler) asyncVerifySignature(arg interface{}) interface{} {
	...
	if tx.verificationErr == nil {
		...
		if err != nil {
			...
			tx.proof, tx.verificationErr = tx.unverifiedTxn.ValidateProof(handler.ledger.VectorCommitments(), transactions.SpecialAddresses{FeeSink: latestHdr.FeeSink, RewardsPool: latestHdr.RewardsPool})
		}
	}
	select {
...
func (handler *TxHandler) processDecoded(unverifiedTxn transactions.SignedTxn) (outmsg network.OutgoingMessage, processingDone bool) {
	...
	// pf, err := tx.unverifiedTxn.ValidateProof(handler.ledger.VectorCommitments(), tx.spec)
	pf, err := tx.unverifiedTxn.PoolValidateProof(handler.ledger.VectorCommitments(), tx.spec, handler.txVerificationPool)
	if err != nil {
		logging.Base().Warnf("Received a txn with a bad proof %v: %v", unverifiedTxn, err)
		return network.OutgoingMessage{Action: network.Disconnect}, true
	}
	...
	// save the transaction, if it has high enough fee and not already in the cache
	err = handler.txPool.Remember(verifiedTxn, pf)
	if err != nil {
		logging.Base().Debugf("could not remember tx: %v", err)
		return network.OutgoingMessage{}, true
	}
