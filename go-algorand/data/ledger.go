...
// BalanceRecord implements agreement.Ledger.BalanceRecord.
// It applies pending rewards to returned amounts.
func (l *Ledger) BalanceRecord(pf basics.UnvalidatedAccountProof) (basics.BalanceRecord, basics.Round, error) {
	data, err := l.ForkSafeLookup(pf, true)
	if err != nil {
		return basics.BalanceRecord{}, 0, err
	}
...
func (*Ledger) AssemblePayset(pool *pools.TransactionPool, eval *ledger.BlockEvaluator, deadline time.Time) (stats telemetryspec.AssembleBlockStats) {
	...
	for len(pending) > 0 {
		txnAndProof := pending[0]
		pending = pending[1:]
		txn := txnAndProof.SignedTxn
		pf := txnAndProof.TransactionProof
		...
		err := eval.Transaction(txn, nil, pf)
	...
	logging.Base().With("EXTPS", true).With("txns", stats.IncludedCount).With("stop", stats.StopReason).Infof("done assembling payset with %v txs. stop reason: %v", stats.IncludedCount, stats.StopReason)
	return
}
