...
type TransactionPool struct {
	...
	pendingTxns  []transactions.SignedTxnAndProof
	pendingTxids map[transactions.Txid]transactions.SignedTxnAndProof
	...
	rememberedTxns  []transactions.SignedTxnAndProof
	rememberedTxids map[transactions.Txid]transactions.SignedTxnAndProof
}
...
func MakeTransactionPool(ledger *ledger.Ledger, transactionPoolStatusSize int, logStats bool, verificationPool execpool.BacklogPool) *TransactionPool {
	pool := TransactionPool{
		pendingTxids:     make(map[transactions.Txid]transactions.SignedTxnAndProof),
		rememberedTxids:  make(map[transactions.Txid]transactions.SignedTxnAndProof),
...
func (pool *TransactionPool) Pending() []transactions.SignedTxnAndProof {
	pool.pendingMu.RLock()
	defer pool.pendingMu.RUnlock()

	txns := make([]transactions.SignedTxnAndProof, len(pool.pendingTxns))
	i := 0
...
func (pool *TransactionPool) rememberCommit(flush bool) {
	...
	pool.rememberedTxids = make(map[transactions.Txid]transactions.SignedTxnAndProof)
}
...
func (pool *TransactionPool) Test(t transactions.SignedTxn, pf transactions.TransactionProof) error {
	t.InitCaches()

	pool.mu.Lock()
	defer pool.mu.Unlock()

	return pool.test(t, pf)
}
...
func (pool *TransactionPool) test(t transactions.SignedTxn, pf transactions.TransactionProof) error {
	...
	err := pool.pendingBlockEvaluator.TestTransaction(t, nil, pf)
...
func (pool *TransactionPool) Remember(t transactions.SignedTxn, pf transactions.TransactionProof) error {
	t.InitCaches()
...
func (pool *TransactionPool) remember(t transactions.SignedTxn, pf transactions.TransactionProof) error {
	err := pool.addToPendingBlockEvaluator(t, pf)
	if err != nil {
		return err
	}

	txpf := transactions.SignedTxnAndProof{SignedTxn: t, TransactionProof: pf}
	pool.rememberedTxns = append(pool.rememberedTxns, txpf)
	pool.rememberedTxids[t.ID()] = txpf
	return nil
}
...
func (pool *TransactionPool) Lookup(txid transactions.Txid) (tx transactions.SignedTxn, txErr string, found bool) {
	...
	entry, inPool := pool.pendingTxids[txid]
	if inPool {
		return entry.SignedTxn, "", true
	}
...
// re-checking signatures and proofs on transactions that we have
// already verified.
func (pool *TransactionPool) Verified(txn transactions.SignedTxn) (transactions.TransactionProof, bool) {
	if pool == nil {
		return transactions.TransactionProof{}, false
	}
	pool.pendingMu.RLock()
	defer pool.pendingMu.RUnlock()
	entry, ok := pool.pendingTxids[txn.ID()]
	if !ok {
		return transactions.TransactionProof{}, false
	}

	pendingSigTxn := entry.SignedTxn
	ok = pendingSigTxn.Sig == txn.Sig
	ok = ok && pendingSigTxn.Msig.Equal(txn.Msig)
	ok = ok && pendingSigTxn.Proof == txn.Proof
...
type alwaysVerifiedPool map[transactions.Txid]transactions.SignedTxnAndProof

func (p alwaysVerifiedPool) Verified(txn transactions.SignedTxn) (transactions.TransactionProof, bool) {
	entry, ok := p[txn.ID()]
	if ok {
		return entry.TransactionProof, true
	}
...
func (pool *TransactionPool) addToPendingBlockEvaluatorOnce(tx transactions.SignedTxn, pf transactions.TransactionProof) error {
	...
	return pool.pendingBlockEvaluator.Transaction(tx, nil, pf)
}

func (pool *TransactionPool) addToPendingBlockEvaluator(tx transactions.SignedTxn, pf transactions.TransactionProof) error {
	err := pool.addToPendingBlockEvaluatorOnce(tx, pf)
	if err == ledger.ErrNoSpace {
...
func (pool *TransactionPool) recomputeBlockEvaluator() (stats telemetryspec.ProcessBlockMetrics) {
	...
	if pool.tailCache != nil {
		tail = pool.tailCache.Tail(latest)
	}

	next := bookkeeping.MakeBlock(prev)
	oldCache := alwaysVerifiedPool(pool.pendingTxids)
	pool.numPendingWholeBlocks = 0
	pool.pendingBlockEvaluator, err = pool.ledger.StartEvaluator(next.BlockHeader, tail, oldCache, pool.verificationPool)
	if err != nil {
	...
	for _, tx := range txns {
		err = pool.remember(tx.SignedTxn, tx.TransactionProof)
		if err != nil {
			pool.statusCache.put(tx.SignedTxn, err.Error())
	...
	logging.Base().Infof("done recomputing block evaluator. num expired: %v. num invalid: %v.", stats.ExpiredCount, stats.RemovedInvalidCount)

	pool.rememberCommit(true)
	return
}

func (pool *TransactionPool) SetTailCache(cache *TailCache) {
	pool.mu.Lock()
	defer pool.mu.Unlock()
	pool.tailCache = cache
}
