...
func BenchmarkAssemblePayset(b *testing.B) {
	...
	backlogPool := execpool.MakeBacklog(nil, 0, execpool.LowPriority, nil) // TODO exec pool
	...
	for i := 0; i < b.N; i++ {
		...
		for tp.PendingCount() < txPoolSize {
			...
			// TODO at some point, we should benchmark with close transactions
			...
			pf, err := signedTx.RefreshProof(l.Ledger, transactions.SpecialAddresses{RewardsPool: poolAddr, FeeSink: sinkAddr})
			if err == nil {
				err = tp.Remember(signedTx, pf)
			}
			if err != nil {
				errcount++
		...
		eval, err := l.StartEvaluator(newEmptyBlk.BlockHeader, basics.AccountTail{}, tp, backlogPool)
		if err != nil {
