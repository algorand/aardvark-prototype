...
func makeVoteTesting(addr basics.Address, vrfSecs *crypto.VRFSecrets, otSecs crypto.OneTimeSigner, ledger Ledger, round basics.Round, period period, step step, digest crypto.Digest) (vote, error) {
	...
	rv := rawVote{Round: round, Period: period, Step: step, Proposal: proposal}
	v, fatalerr := makeVote(rv, otSecs, vrfSecs, dummyAccountProof(addr, round), ledger)
	if fatalerr != nil {
		panic(fatalerr)
	}
...
func TestVoteValidation(t *testing.T) {
	...
	for i, address := range addresses[:numAddresses] {
		...
		rv := rawVote{Round: round, Period: period, Step: step(i), Proposal: proposal}
		unauthenticatedVote, err := makeVote(rv, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
		require.NoError(t, err)

		m, err := membership(ledger, round, period, step(i), dummyAccountProof(address, round))
		require.NoError(t, err)
...
func TestVoteReproposalValidation(t *testing.T) {
	...
	for i, address := range addresses[:numAddresses] {
		...
		rv := rawVote{Round: round, Period: per, Step: step(0), Proposal: proposal}
		unauthenticatedVote, err := makeVote(rv, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
		require.NoError(t, err)

		m, err := membership(ledger, round, per, step(0), dummyAccountProof(address, round))
		require.NoError(t, err)
		...
		if selected {
			...
			reproposalVote, err := makeVote(rv, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
			require.NoError(t, err)
			...
			rv.Proposal.OriginalProposer = basics.Address(randomBlockHash())
			badReproposalVote, err := makeVote(rv, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
			require.NoError(t, err)
...
func TestVoteMakeVote(t *testing.T) {
	...
	rv := rawVote{Round: round, Period: period, Step: step(addressIndex), Proposal: proposal}
	unauthenticatedVote, err := makeVote(rv, otSecrets[addressIndex], vrfSecrets[addressIndex], dummyAccountProof(address, round), ledger)
	require.NoError(t, err)
	require.NotNil(t, unauthenticatedVote)
	...
	rv = rawVote{Round: round, Period: period, Step: step(addressIndex), Proposal: proposal}
	unauthenticatedVote, err = makeVote(rv, otSecrets[addressIndex], vrfSecrets[addressIndex], dummyAccountProof(basics.Address{}, round), ledger)
	//require.Error(t, err)

	//  creating a vote in cert and bottom mode results in panic.
	addressIndex++
	address = addresses[addressIndex]
	rv = rawVote{Round: round, Period: period, Step: cert, Proposal: bottom}
	makeVotePanicWrapper(t, "makeVote: votes from step 2 cannot validate bottom", rv, address, otSecrets[addressIndex], vrfSecrets[addressIndex], ledger)

}

func makeVotePanicWrapper(t *testing.T, message string, rv rawVote, sender basics.Address, voting crypto.OneTimeSigner, selection *crypto.VRFSecrets, l Ledger) (uav unauthenticatedVote, err error) {
	logging.Base().SetOutput(nullWriter{})
	require.Panics(t, func() { uav, err = makeVote(rv, voting, selection, dummyAccountProof(sender, rv.Round), l) })
...
func TestVoteValidationStepCertAndProposalBottom(t *testing.T) {
	...
	for i, address := range addresses {
		...
		//  creating a vote in cert and bottom mode results in panic.
		rawVote := rawVote{Round: round, Period: period, Step: step(i), Proposal: proposal}
		unauthenticatedVote, err := makeVote(rawVote, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
...
func TestEquivocationVoteValidation(t *testing.T) {
	...
	for i, address := range addresses[:numAddresses] {
		...
		rv0 := rawVote{Round: round, Period: period, Step: step(i), Proposal: proposal1}
		unauthenticatedVote0, err := makeVote(rv0, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
		require.NoError(t, err)

		rv0Copy := rawVote{Round: round, Period: period, Step: step(i), Proposal: proposal1}
		unauthenticatedVote0Copy, err := makeVote(rv0Copy, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
		require.NoError(t, err)

		var proposal2 proposalValue
		proposal2.BlockDigest = randomBlockHash()
		rv1 := rawVote{Round: round, Period: period, Step: step(i), Proposal: proposal2}
		unauthenticatedVote1, err := makeVote(rv1, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
		require.NoError(t, err)
		...
		ev := unauthenticatedEquivocationVote{
			...
			Record:    dummyAccountProof(address, round),
		...
		if selected {
			...
			badSender := ev
			badSender.Record.Address = basics.Address{}
