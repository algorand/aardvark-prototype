...
type unauthenticatedProposal struct {
	...
	OriginalPeriod        period                         `codec:"oper"`
	OriginalProposerProof basics.UnvalidatedAccountProof `codec:"opf"`
}
...
func (p unauthenticatedProposal) value() proposalValue {
	return proposalValue{
		...
		OriginalProposer: p.OriginalProposerProof.Address,
...
func makeProposal(ve ValidatedBlock, pf crypto.VrfProof, origPer period, origPf basics.UnvalidatedAccountProof) proposal {
	e := ve.Block()
	var payload unauthenticatedProposal
	payload.Block = e
	payload.SeedProof = pf
	payload.OriginalPeriod = origPer
	payload.OriginalProposerProof = origPf
...
func verifyNewSeed(p unauthenticatedProposal, ledger LedgerReader) error {
	...
	_, recordRound, err := ledger.BalanceRecord(p.OriginalProposerProof)
	if err != nil {
		return fmt.Errorf("failed to obtain balance record for address %v in round %v: %v", value.OriginalProposer, recordRound, err)
	}
	if balanceRound(rnd, cparams) != recordRound {
		return fmt.Errorf("balanceRound %d mismatches proposal record proof round %d: %v", balanceRound(rnd, cparams), recordRound, err)
	}
...
func proposalForBlock(pf basics.UnvalidatedAccountProof, vrf *crypto.VRFSecrets, ve ValidatedBlock, period period, ledger LedgerReader) (proposal, proposalValue, error) {
	address := pf.Address
	rnd := ve.Block().Round()
	...
	proposal := makeProposal(ve, seedProof, period, pf)
	value := proposalValue{
...
func (p unauthenticatedProposal) validate(ctx context.Context, current round, ledger LedgerReader, validator BlockValidator) (proposal, error) {
	...
	return makeProposal(ve, p.SeedProof, p.OriginalPeriod, p.OriginalProposerProof), nil
}
