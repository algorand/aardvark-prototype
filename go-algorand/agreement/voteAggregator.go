...
func (agg *voteAggregator) filterVote(proto protocol.ConsensusVersion, p player, r routerHandle, uv unauthenticatedVote, freshData freshnessData) error {
	...
	filterReq := voteFilterRequestEvent{Vote: uv}
	...
	case voteFilteredStep:
		...
		return fmt.Errorf("voteAggregator: rejected vote: sender %v had already sent a vote in round %v period %v step %v", uv.sender(), uv.R.Round, uv.R.Period, uv.R.Step)
	case none:
		return nil
	}
