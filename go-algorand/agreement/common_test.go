...
func dummyAccountProof(addr basics.Address, current basics.Round) (pf basics.UnvalidatedAccountProof) {
	pf.Address = addr
	pf.Round = balanceRound(current, config.Consensus[protocol.ConsensusCurrentVersion])
	return pf
}
...
func (l *testLedger) BalanceRecord(pf basics.UnvalidatedAccountProof) (basics.BalanceRecord, basics.Round, error) {
	l.mu.Lock()
	defer l.mu.Unlock()

	if pf.Round >= l.nextRound {
		err := fmt.Errorf("BalanceRecord called on future round: %v >= %v! (this is probably a bug)", pf.Round, l.nextRound)
		panic(err)
	}
	return l.state[pf.Address], pf.Round, nil
}
...
func makeProposalsTesting(accs testAccountData, round basics.Round, period period, factory BlockFactory, ledger Ledger) (ps []proposal, vs []vote) {
	...
	for i := range accs.addresses {
		payload, proposal, err := proposalForBlock(dummyAccountProof(accs.addresses[i], round), accs.vrfs[i], ve, period, ledger)
		if err != nil {
		...
		uv, err := makeVote(rv, accs.ots[i], accs.vrfs[i], dummyAccountProof(accs.addresses[i], round), ledger)
		if err != nil {
...
func (v *voteMakerHelper) MakeRawVote(t *testing.T, r round, p period, s step, value proposalValue) rawVote {
	return rawVote{Round: r, Period: p, Step: s, Proposal: value}
}
...
func (v *voteMakerHelper) MakeVerifiedVote(t *testing.T, index int, r round, p period, s step, value proposalValue) vote {
	...
	return vote{
		R:      v.MakeRawVote(t, r, p, s, value),
		Sender: v.addresses[index],
		Record: dummyAccountProof(v.addresses[index], r),
