...
func TestVoteTrackerFiltersDuplicateVoteOnce(t *testing.T) {
	...
	for i := 0; i < int(Num+1); i++ {
		...
		case i < Num:
			...
			expectedOutputs[i] = thresholdEvent{T: none}
		case i == Num:
			inputVotes[i] = voteFilterRequestEvent{Vote: inputVotes[Num-1].(voteAcceptedEvent).Vote.u()}
...
func TestVoteTrackerForwardsFirstEquivocation(t *testing.T) {
	...
	for i := 0; i < int(V1V2Bound+1); i++ {
		...
		case i == V1Bound:
			// simple duplicate
			inputVotes[i] = voteFilterRequestEvent{Vote: inputVotes[i-1].(voteAcceptedEvent).Vote.u()}
			expectedOutputs[i] = filteredStepEvent{T: voteFilteredStep}
		case i < V2Bound:
		...
		case i == V2Bound:
			// simple duplicate
			uv := inputVotes[i-1].(voteAcceptedEvent).Vote.u()
			inputVotes[i] = voteFilterRequestEvent{Vote: uv}
			require.EqualValuesf(t, equivVal2, uv.R.Proposal, "test case is malformed, filtering incorrect vote")
			expectedOutputs[i] = filteredStepEvent{T: voteFilteredStep}
		case i == V2Bound+1:
			// make sure first equivocation is not filtered
			voteTwo := helper.MakeValidVoteAcceptedVal(t, V2Bound-1, next, equivVal1)
			inputVotes[i] = voteFilterRequestEvent{Vote: voteTwo.Vote.u()}
		...
		case i == V1V2Bound:
			...
			inputVotes[i] = voteFilterRequestEvent{Vote: voteThree.Vote.u()}
			expectedOutputs[i] = filteredStepEvent{T: voteFilteredStep}
		}
	}
	testCase := determisticTraceTestCase{
...
func TestVoteTrackerFiltersFutureEquivocations(t *testing.T) {
	...
	for i := 0; i < int(Num); i++ {
		...
		case i == 1:
			...
			inputVotes[i] = voteFilterRequestEvent{Vote: VA.Vote.u()}
			expectedOutputs[i] = emptyEvent{}
		case i == 2:
			// add an equivocation
			Val := proposalValue{BlockDigest: randomBlockHash()}
