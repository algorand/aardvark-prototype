...
type pseudonodeBaseTask struct {
	...
	participation []account.ParticipationData
}
...
func (n asyncPseudonode) getParticipations(procName string, round basics.Round) []account.ParticipationData {
	cparams, err := n.ledger.ConsensusParams(ParamsRound(round))
	if err != nil {
		n.log.Errorf("%v (round=%v): Could not consensus params: %v", procName, round, err)
		return nil
	}

	return n.keys.Keys(balanceRound(round, cparams))
}

// makeProposals creates a slice of block proposals for the given round and period.
func (n asyncPseudonode) makeProposals(round basics.Round, period period, accounts []account.ParticipationData) ([]proposal, []unauthenticatedVote) {
	...
	for _, partData := range accounts {
		account, pf := partData.Participation, partData.AccountProof.Unvalidated()
		payload, proposal, err := proposalForBlock(pf, account.VRFSecrets(), ve, period, n.ledger)
		if err != nil {
		...
		rv := rawVote{Round: round, Period: period, Step: propose, Proposal: proposal}
		uv, err := makeVote(rv, account.VotingSigner(), account.VRFSecrets(), pf, n.ledger)
		if err != nil {
...
func (n asyncPseudonode) makeVotes(round basics.Round, period period, step step, proposal proposalValue, participation []account.ParticipationData) []unauthenticatedVote {
	votes := make([]unauthenticatedVote, 0)
	for _, partData := range participation {
		account := partData.Participation
		rv := rawVote{Round: round, Period: period, Step: step, Proposal: proposal}
		uv, err := makeVote(rv, account.VotingSigner(), account.VRFSecrets(), partData.AccountProof.Unvalidated(), n.ledger)
		if err != nil {
