...
func createProposalsTesting(accs testAccountData, round basics.Round, period period, factory BlockFactory, ledger Ledger) (ps []proposal, vs []vote) {
	...
	for i := range accs.addresses {
		payload, proposal, _ := proposalForBlock(dummyAccountProof(accs.addresses[i], round), accs.vrfs[i], ve, period, ledger)
...
func TestProposalFunctions(t *testing.T) {
	...
	for i := range accs.addresses {
		proposal, proposalValue, _ := proposalForBlock(dummyAccountProof(accs.addresses[i], round), accs.vrfs[i], ve, period, ledger)
...
func TestProposalUnauthenticated(t *testing.T) {
	...
	proposal3, _, _ := proposalForBlock(dummyAccountProof(accounts.addresses[accountIndex], round), accounts.vrfs[accountIndex], testBlockFactory, period, ledger)
	accountIndex++
