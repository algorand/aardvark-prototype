...
func sortedVoteGen(t *testing.T) (votes []vote) {
	...
	for i, addr := range addresses {
		...
		rv := rawVote{Round: ledger.NextRound(), Proposal: pv}
		uv, err := makeVote(rv, ots[i], vrfs[i], dummyAccountProof(addr, rv.Round), ledger)
...
func (s *proposalTrackerTestShadow) addVote(v vote) {
	...
	// check seen before
	req = voteFilterRequestEvent{Vote: v.u()}
...
func setupProposalTrackerTests(t *testing.T) (votes []vote) {
	...
	for i := range addrs {
		...
		uv, err := makeVote(rv, ots[i], vrfs[i], dummyAccountProof(addrs[i], rv.Round), ledger)
		require.NoError(t, err)
