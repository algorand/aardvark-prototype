...
func (a voteAuthenticator) sender() basics.Address {
	return a.Record.Address
}

func (a equivocationVoteAuthenticator) sender() basics.Address {
	return a.Record.Address
}
...
func (b unauthenticatedBundle) verifyAsync(ctx context.Context, l LedgerReader, avv *AsyncVoteVerifier) func() (bundle, error) {
	...
	for i, auth := range b.Votes {
		...
		rv := rawVote{Round: b.Round, Period: b.Period, Step: b.Step, Proposal: b.Proposal}
		uv := unauthenticatedVote{R: rv, Cred: auth.Cred, Record: auth.Record, Sig: auth.Sig}
