...
func TestBundleCreation(t *testing.T) {
	...
	for s := 1; s <= 4; s++ {
		...
		for i := range addresses {
			...
			uv, err := makeVote(rv, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
...
func TestBundleCreationWithVotesFromSameAddress(t *testing.T) {
	...
	for s := 1; s <= 5; s++ {
		...
		for i := range addresses {
			...
			rv0 := rawVote{Round: round, Period: period, Step: step, Proposal: proposal}
			uv0, err := makeVote(rv0, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
			require.NoError(t, err)
			...
			rv1 := rawVote{Round: round, Period: period, Step: step, Proposal: proposal2}
			uv1, err := makeVote(rv1, otSecrets[i], vrfSecrets[i], dummyAccountProof(address, round), ledger)
			require.NoError(t, err)
...
func TestBundleCreationWithEquivocationVotes(t *testing.T) {
	...
	voteBadSenderBundle.Votes[0].Record.Address = basics.Address{}
