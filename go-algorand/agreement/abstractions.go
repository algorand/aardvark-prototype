...
type LedgerReader interface {
	...
	// BalanceRecord returns the BalanceRecord associated with some proof
	// and the round to which it corresponds.
	//
	// This method returns an error if the provided AccountProof is
	// insufficient for computing the address's BalanceRecord.  In this
	// case, all other correct and up-to-date players will obtain an error.
	//
	// This method also returns an error if the proof's Round has not yet been
	// confirmed. It may also return an error if the given Round is
	// unavailable by the storage device.  In this case, it is possible that
	// some other correct and up-to-date players will not obtain an error
	// here.  If such errors are pervasive, the protocol can lose liveness.
	BalanceRecord(basics.UnvalidatedAccountProof) (basics.BalanceRecord, basics.Round, error)
...
type KeyManager interface {
	// Keys returns a slice of accounts participating in the given round,
	// along with proofs of their balance records.
	Keys(basics.Round) []account.ParticipationData
