...
func TestBlockAssemblerPipeline(t *testing.T) {
	...
	accountIndex := 0
	proposal, _, _ := proposalForBlock(dummyAccountProof(accounts.addresses[accountIndex], round), accounts.vrfs[accountIndex], testBlockFactory, period, ledger)
	accountIndex++
...
func TestBlockAssemblerBind(t *testing.T) {
	...
	proposal, _, _ := proposalForBlock(dummyAccountProof(accounts.addresses[accountIndex], player.Round), accounts.vrfs[accountIndex], testBlockFactory, player.Period, ledger)
	accountIndex++
...
func TestBlockAssemblerAuthenticator(t *testing.T) {
	...
	require.NoError(t, err, "Could not generate a proposal for round %v: %v", player.Round, err)
	accountIndex := 0
	proposalPayload, _, _ := proposalForBlock(dummyAccountProof(accounts.addresses[accountIndex], player.Round), accounts.vrfs[accountIndex], testBlockFactory, player.Period, ledger)
...
func TestProposalStoreT(t *testing.T) {
	...
	require.NoError(t, err, "Could not generate a proposal for round %v: %v", player.Round, err)
	accountIndex := 0
	proposalPayload, proposalV, _ := proposalForBlock(dummyAccountProof(accounts.addresses[accountIndex], player.Round), accounts.vrfs[accountIndex], testBlockFactory, player.Period, ledger)
...
func TestProposalStoreHandle(t *testing.T) {
	...
	require.NoError(t, err, "Could not generate a proposal for round %v: %v", player.Round, err)
	accountIndex := 0
	_, proposalV0, _ := proposalForBlock(dummyAccountProof(accounts.addresses[accountIndex], player.Round), accounts.vrfs[accountIndex], testBlockFactory, player.Period, ledger)
	accountIndex++
	proposalPayload, proposalV, _ := proposalForBlock(dummyAccountProof(accounts.addresses[accountIndex], player.Round), accounts.vrfs[accountIndex], testBlockFactory, player.Period, ledger)
...
func TestProposalStoreGetPinnedValue(t *testing.T) {
	...
	payloadV, proposalV, _ := proposalForBlock(dummyAccountProof(accounts.addresses[accountIndex], player.Round), accounts.vrfs[accountIndex], testBlockFactory, player.Period, ledger)
...
func TestProposalStoreRegressionBlockRedeliveryBug_b29ea57(t *testing.T) {
	...
	propPay = proposal{
		unauthenticatedProposal: unauthenticatedProposal{
			OriginalPeriod:        1,
			OriginalProposerProof: dummyAccountProof(proposer, curRound),
		},
	}
	...
	msgV1 = message{
		...
		Vote:                vote{R: rv},
		UnauthenticatedVote: unauthenticatedVote{R: rv, Record: dummyAccountProof(proposer, rv.Round)},
	}
	msgP1 = message{
		Tag:                     protocol.ProposalPayloadTag,
		Proposal:                propPay,
	...
	propPay = proposal{
		unauthenticatedProposal: unauthenticatedProposal{
			OriginalPeriod:        2,
			OriginalProposerProof: dummyAccountProof(proposer, rv.Round),
		},
	}
	propVal = proposalValue{
		OriginalPeriod:   2,
