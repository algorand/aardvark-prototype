...
func (l *testLedger) BalanceRecord(a basics.UnvalidatedAccountProof) (basics.BalanceRecord, basics.Round, error) {
	l.mu.Lock()
	defer l.mu.Unlock()

	if a.Round >= l.nextRound {
		err := fmt.Errorf("BalanceRecord called on future round: %v >= %v! (this is probably a bug)", a.Round, l.nextRound)
		panic(err)
	}
	return l.state[a.Address], a.Round, nil
}
