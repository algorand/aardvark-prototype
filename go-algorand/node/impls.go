...
type blockFactoryImpl struct {
	l                *data.Ledger
	tp               *pools.TransactionPool
	tailCache        *pools.TailCache
	logStats         bool
	verificationPool execpool.BacklogPool
}

func makeBlockFactory(l *data.Ledger, tp *pools.TransactionPool, tailCache *pools.TailCache, logStats bool, executionPool execpool.BacklogPool) *blockFactoryImpl {
	bf := &blockFactoryImpl{
		l:                l,
		tp:               tp,
		tailCache:        tailCache,
...
func (i *blockFactoryImpl) AssembleBlock(round basics.Round, deadline time.Time) (agreement.ValidatedBlock, error) {
	...
	eval, err := i.l.StartEvaluator(newEmptyBlk.BlockHeader, i.tailCache.Tail(round-1), i.tp, i.verificationPool)
	if err != nil {
...
// TODO put this in config.Local
const archiveFetchMaxWeightTime = 5 * time.Second

type archive struct {
	local  *ledger.Ledger
	remote rpcs.AccountFetcher

	// if cache is set, local durably tracks proofs returned by remote
	cache bool
}

func (a archive) validateAccountProof(rnd basics.Round, addr basics.Address, unvalidated basics.UnvalidatedAccountProof) (basics.AccountProof, error) {
	pf, err := a.local.VerifyAccountProof(unvalidated)
	if err != nil {
		return basics.AccountProof{}, fmt.Errorf("verifying unvalidated proof failed: %v", err)
	}

	if pf.Round != rnd {
		return basics.AccountProof{}, fmt.Errorf("pf.Round %d != rnd %d", pf.Round, rnd)
	}
	if pf.Address != addr && !pf.Surrounds(addr) {
		return basics.AccountProof{}, fmt.Errorf("pf.Address %v != addr %v (and pf does not surround; pf.Next %v)", pf.Address, addr, pf.Next)
	}

	return pf, nil
}

func (a archive) validatePrevAccountProof(rnd basics.Round, addr basics.Address, unvalidated basics.UnvalidatedAccountProof) (basics.AccountProof, error) {
	pf, err := a.local.VerifyAccountProof(unvalidated)
	if err != nil {
		return basics.AccountProof{}, fmt.Errorf("verifying unvalidated proof failed: %v", err)
	}
	...
	if pf.Next != addr {
		// indicates that account has already been deleted
		// do not drop node as this may happen due to concurrent updates
		return pf, nil
	}
...
// LookupProof implements transactions.Archive.
func (a archive) LookupProof(rnd basics.Round, addr basics.Address) (basics.AccountProof, error) {
	pf, err := a.local.LookupProof(rnd, addr)
	if err == nil {
		return pf, nil
	}

	ctx, cancel := context.WithTimeout(context.Background(), archiveFetchMaxWeightTime)
	defer cancel()
	pf, err = a.remote.FetchAccount(ctx, rnd, addr, a.validateAccountProof)
	if a.cache && err == nil {
		a.local.CacheProof(pf)
	}
	return pf, err
}

// LookupPrevProof implements transactions.Archive.
func (a archive) LookupPrevProof(rnd basics.Round, addr basics.Address) (pf basics.AccountProof, err error) {
	defer func() {
		if err == nil && pf.Next != addr {
			pf = basics.AccountProof{}
			err = fmt.Errorf("address %v has already been deleted as of round %d", addr, pf.Round)
		}
	}()

	pf, err = a.local.LookupPrevProof(rnd, addr)
	if err == nil {
		return pf, nil
	}
	...
	ctx, cancel := context.WithTimeout(context.Background(), archiveFetchMaxWeightTime)
	defer cancel()
	pf, err = a.remote.FetchPrevAccount(ctx, rnd, addr, a.validatePrevAccountProof)
	if a.cache && err == nil {
		a.local.CacheProof(pf)
	}
	return pf, err
