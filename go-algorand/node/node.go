...
type AlgorandFullNode struct {
	...
	accountService          *rpcs.AccountService
	wsAccountFetcherService *rpcs.WsAccountFetcherService
	accountFetcher          rpcs.AccountFetcher
...
func MakeFull(log logging.Logger, rootDir string, cfg config.Local, phonebookDir string, genesis bookkeeping.Genesis) (*AlgorandFullNode, error) {
	...
	addrs, err := config.LoadPhonebook(rootDir)
	if err != nil {
	...
	node.transactionPool = pools.MakeTransactionPool(node.ledger.Ledger, cfg.TxPoolSize, cfg.EnableAssembleStats, node.lowPriorityCryptoVerificationPool)
	if !cfg.Archival {
	...
	node.accountService = rpcs.RegisterAccountService(cfg, node.ledger.Ledger, p2pNode)
	node.wsAccountFetcherService = rpcs.RegisterWsAccountFetcherService(cfg, p2pNode)
	node.accountFetcher = rpcs.MakeAccountFetcher(p2pNode, node.wsAccountFetcherService)
	rpcs.RegisterTxService(node.transactionPool, p2pNode, node.genesisID, cfg.TxPoolSize, cfg.TxSyncServeResponseSize)

	node.tailCache = pools.OpenTailCache(node.accountFetcher, node.ledger)
	node.transactionPool.SetTailCache(node.tailCache)
	node.accountManager = data.MakeAccountManager(log, archive{local: node.ledger.Ledger, remote: node.accountFetcher, cache: true})

	node.ledger.RegisterBlockListeners([]ledger.BlockListener{node.tailCache, node.transactionPool}) // note: tailCache must come _before_ transactionPool here

	latest := node.ledger.Latest()
	hdr, err := node.ledger.BlockHdr(latest)
	if err != nil {
		log.Warnf("rewards pool hack: block header fetch failed: %v", err)
	} else {
		node.accountManager.XXXsetSpecialAddresses(hdr.RewardsPool, hdr.FeeSink)
	}
	...
	blockFactory := makeBlockFactory(node.ledger, node.transactionPool, node.tailCache, node.config.EnableProcessBlockStats, node.highPriorityCryptoVerificationPool)
...
func (node *AlgorandFullNode) Stop() {
	...
	node.tailCache.Close()
	node.ledgerService.Stop()
	node.accountService.Stop()
	node.accountFetcher.Close()
...
func (node *AlgorandFullNode) BroadcastSignedTxn(signed transactions.SignedTxn) (transactions.Txid, error) {
	...
	pf, err := signed.RefreshProof(archive{local: node.ledger.Ledger, remote: node.accountFetcher, cache: false}, spec)
	if err != nil {
		node.log.Infof("failed to refresh proof: %v - transaction was %+v", err, signed)
		return transactions.Txid{}, err
	}
	err = node.transactionPool.Remember(signed, pf)
	if err != nil {
...
func (node *AlgorandFullNode) GetPendingTxnsFromPool() ([]transactions.SignedTxn, error) {
	txpfs := node.transactionPool.Pending()
	txns := make([]transactions.SignedTxn, len(txpfs))
	for i, txpf := range txpfs {
		txns[i] = txpf.SignedTxn
	}
	return txns, nil
}
