...
type MockRunner struct {
	...
	failWithNil   bool
	failWithError bool
	transactions  []transactions.SignedTxnAndProof
}
...
func (client *MockRPCClient) Sync(ctx context.Context, bloom *bloom.Filter) (txns []transactions.SignedTxn, err error) {
	...
	txns = make([]transactions.SignedTxn, len(client.client.transactions))
	for i := range txns {
		txns[i] = client.client.transactions[i].SignedTxn
	}
	return txns, nil
}
