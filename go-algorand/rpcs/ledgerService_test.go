...
func TestGetBlockMocked(t *testing.T) {
	...
	pf, err := tx.Prove(ledgerA, transactions.SpecialAddresses{FeeSink: sinkAddr, RewardsPool: poolAddr})
	require.NoError(t, err)
...
func buildTestLedger(t *testing.T) (ledger *data.Ledger, next basics.Round, b bookkeeping.Block, err error) {
	...
	pf, err := tx.Prove(ledger, transactions.SpecialAddresses{FeeSink: sinkAddr, RewardsPool: poolAddr})
	require.NoError(t, err)
	signedtx.Proof = pf.Unvalidated()
