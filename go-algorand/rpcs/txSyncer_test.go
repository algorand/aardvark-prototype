...
type mockPendingTxAggregate struct {
	txns []transactions.SignedTxnAndProof
}
...
func makeMockPendingTxAggregate(txCount int) mockPendingTxAggregate {
	...
	mock := mockPendingTxAggregate{
		txns: make([]transactions.SignedTxnAndProof, txCount),
	}
	...
	for i := 0; i < txCount; i++ {
		...
		stx := tx.Sign(sk)
		stx.InitCaches()
		mock.txns[i] = transactions.SignedTxnAndProof{SignedTxn: stx}
	}
	return mock
}
...
func (mock mockPendingTxAggregate) Pending() []transactions.SignedTxnAndProof {
	signedTxns := make([]transactions.SignedTxnAndProof, 0)
	for _, tx := range mock.txns {
