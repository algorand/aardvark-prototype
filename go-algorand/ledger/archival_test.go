...
var shortTxnLifeProto = protocol.ConsensusCurrentVersion

func init() {
	// to speed up tests, test protocol has shorter max txn life
	shortTxnLifeProto = "TestShortMaxTxnLife-" + protocol.ConsensusCurrentVersion
	params := config.Consensus[protocol.ConsensusCurrentVersion]
	params.MaxTxnLife = 320
	config.Consensus[shortTxnLifeProto] = params
}
...
// 		minMinSave, err := checkTrackers(t, wl, blk.Round(), proto)
// 		require.NoError(t, err)
// 		if err != nil {
...
func checkTrackers(t *testing.T, wl *wrappedLedger, rnd basics.Round, proto protocol.ConsensusVersion) (basics.Round, error) {
	minMinSave := rnd
	...
	for _, trk := range wl.l.trackers.trackers {
		...
		if trackerType.String() == "ledger.accountUpdates" {
			...
			cleanTracker.(*accountUpdates).initProto = config.Consensus[proto]
		} else if trackerType.String() == "ledger.acctProofTracker" {
			cleanTracker.(*acctProofTracker).initProto = config.Consensus[proto]
		}
