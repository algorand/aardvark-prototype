...
func init() {
	testDefaultVectorParams = vector.MakeParameters(1000)
}

var testDefaultVectorParams *vector.Parameters
...
func randomAccountData(rewardsLevel uint64) basics.AccountData {
	...
	data.MicroAlgos.Raw = crypto.RandUint64() % (1 << 16)
...
func checkAccounts(t *testing.T, tx *sql.Tx, rnd basics.Round, accts map[basics.Address]basics.AccountData) {
	r, _, err := accountsRound(tx) // TODO check residue
...
func TestAccountDBInit(t *testing.T) {
	...
	err = accountsInit(tx, accts, proto, testDefaultVectorParams)
	require.NoError(t, err)
...
func TestAccountDBRound(t *testing.T) {
	...
	for i := 1; i < 10; i++ {
		...
		accts = newaccts
		sd := slotDelta{} // TODO check integrity of slot updates
		sr := slotResidue{}
		err = accountsNewRound(tx, basics.Round(i), updates, sd, 0, sr, proto)
		require.NoError(t, err)
